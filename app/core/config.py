...
def _worker_count() -> int:
    value = (os.getenv("WORKER_COUNT") or os.getenv("WORKER_MODE") or "38").strip().lower()
    try:
...
class Settings:
    ...
    min_ready_workers: int = _int("MIN_READY_WORKERS", 20)
    ...
    auto_rotate_min_ready_workers: int = _int("AUTO_ROTATE_MIN_READY_WORKERS", 20)
    auto_rotate_max_parallel: int = _int("AUTO_ROTATE_MAX_PARALLEL", 3)
    auto_rotate_max_parallel_per_token: int = _int("AUTO_ROTATE_MAX_PARALLEL_PER_TOKEN", 3)
    auto_rotate_target_attempts: int = _int("AUTO_ROTATE_TARGET_ATTEMPTS", 2)
    auto_rotate_connect_timeout_seconds: int = _int("AUTO_ROTATE_CONNECT_TIMEOUT_SECONDS", 30)
    auto_rotate_max_uptime_seconds: int = _int("AUTO_ROTATE_MAX_UPTIME_SECONDS", 70)
    ...
    health_interval_seconds: int = _int("HEALTH_INTERVAL_SECONDS", 8)
    ...
    worker_recovery_connect_after_failures: int = _int("WORKER_RECOVERY_CONNECT_AFTER_FAILURES", 2)
    worker_recovery_restart_after_failures: int = _int("WORKER_RECOVERY_RESTART_AFTER_FAILURES", 20)
    worker_recovery_docker_restart_after_failures: int = _int("WORKER_RECOVERY_DOCKER_RESTART_AFTER_FAILURES", 40)
    worker_recovery_cooldown_seconds: int = _int("WORKER_RECOVERY_COOLDOWN_SECONDS", 30)
    worker_recovery_verify_seconds: int = _int("WORKER_RECOVERY_VERIFY_SECONDS", 15)
    worker_recovery_connect_max_attempts: int = _int("WORKER_RECOVERY_CONNECT_MAX_ATTEMPTS", 1)
    worker_recovery_max_parallel: int = _int("WORKER_RECOVERY_MAX_PARALLEL", 2)
    docker_recovery_enabled: bool = _bool("DOCKER_RECOVERY_ENABLED", False)
    ...
    ready_failure_threshold: int = _int("READY_FAILURE_THRESHOLD", 5)
    ready_verified_grace_seconds: int = _int("READY_VERIFIED_GRACE_SECONDS", 60)
    ...
    auto_rotate_pause_ready_threshold: int = _int("AUTO_ROTATE_PAUSE_READY_THRESHOLD", 20)
