import pexpect

child = pexpect.spawn('ssh -o StrictHostKeyChecking=no root@103.82.193.113', timeout=120, encoding='utf-8')
child.expect('[pP]assword:')
child.sendline('YFg8cWa54YTogkW9')
child.expect('root@')

script = """
import subprocess, time, random

# 1. Lay danh sach tat ca cac regions tu PIA
res_regions = subprocess.run(["docker", "exec", "pia-worker-1", "piactl", "get", "regions"], capture_output=True, text=True)
all_regions = [r.strip() for r in res_regions.stdout.splitlines() if r.strip() and r.strip() != "auto"]

# 2. Xoay worker 17, 18, 19, 20 vao cac region ngau nhien khac nhau
for w in [17, 18, 19, 20]:
    target = random.choice(all_regions)
    print(f"Setting Worker {w} to region: {target}")
    subprocess.run(["docker", "exec", f"pia-worker-{w}", "piactl", "set", "region", target], capture_output=True)
    subprocess.run(["docker", "exec", f"pia-worker-{w}", "piactl", "connect"], capture_output=True)
    time.sleep(1)

time.sleep(5)
print("=== CHECK 20 LIVE WORKERS STATUS & REGIONS ===")
for i in range(1, 21):
    res_ip = subprocess.run(["docker", "exec", f"pia-worker-{i}", "piactl", "get", "vpnip"], capture_output=True, text=True)
    res_reg = subprocess.run(["docker", "exec", f"pia-worker-{i}", "piactl", "get", "region"], capture_output=True, text=True)
    res_st = subprocess.run(["docker", "exec", f"pia-worker-{i}", "piactl", "get", "connectionstate"], capture_output=True, text=True)
    print(f"Worker {i:02d}: IP={res_ip.stdout.strip():<16} | Region={res_reg.stdout.strip():<20} | State={res_st.stdout.strip()}")
"""

import base64
b64 = base64.b64encode(script.encode()).decode()
child.sendline(f"echo '{b64}' | base64 -d > /tmp/fix_regions.py && python3 /tmp/fix_regions.py")
child.expect('Worker 20:', timeout=90)
print(child.before)
child.expect('root@', timeout=30)
print(child.before)
child.close()
