...
Thư mục này chứa toàn bộ cấu hình, script tối ưu hóa và `docker-compose.yml` để chạy **32 Worker PIA SOCKS5 Proxy** ổn định trên máy chủ cấu hình tối thiểu (1 vCPU, 1GB RAM, 20GB Disk) với cơ chế **Zero-Leak Protection (tuyệt đối không rò rỉ IP máy chủ)**.
...
- **Điều phối 1 CPU Core:**
  ...
  - Khởi động so le (Staggered boot) chống quá tải lúc mở 32 đường hầm VPN.
